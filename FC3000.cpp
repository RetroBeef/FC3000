...
void FC3000::setTime(const uint32_t& unixtime) {
  String command =  "6|1|20631|ALL|" + String(unixtime);//todo get 20631 from name
  writeCharacteristic->writeValue((uint8_t*)command.c_str(), command.length(), true);
}

void FC3000::setTimer(const String & startTimeHHMM, const uint16_t durationMinutes) {
  String command = "60|0|" + deviceId + "|1|" + startTimeHHMM + "," + String((int)durationMinutes) + "|0";
  writeCharacteristic->writeValue((uint8_t*)command.c_str(), command.length(), true);
}
